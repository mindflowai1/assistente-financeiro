...
const ThemeSwitcher: React.FC = () => {
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`relative w-14 h-8 flex items-center rounded-full p-1 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 ${theme === 'dark' ? 'bg-green-600 focus:ring-green-500 focus:ring-offset-gray-800' : 'bg-gray-300 focus:ring-green-600 focus:ring-offset-white'
        }`}
      aria-label={`Switch to ${theme === 'light' ? 'dark' : 'light'} mode`}
    >
      <span
        className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-300 ease-in-out flex items-center justify-center ${theme === 'dark' ? 'translate-x-6' : 'translate-x-0'
          }`}
      >
        {/* Sun Icon */}
...
export const AppLayout: React.FC = () => {
  ...
  const [isAdmin, setIsAdmin] = useState(false); // Added this line
  ...
  const pageTitles: { [key: string]: string } = {
    ...
    '/admin': 'Administração', // Added this line
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="flex justify-between items-center h-20">
            <div className="flex items-center gap-4">
              <Link to="/dashboard">
                <img
                  src="/images/logo assistente fav icon.png"
                  alt="Logo"
                  className="h-10 w-10"
                />
              </Link>
            ...
            <div className="flex items-center gap-4">
              ...
              <nav className="hidden md:flex items-center gap-2">
                {menuItems.map((item) => (
                  <Link key={item.path} to={item.path} className={desktopMenuItemClass(item.path)}>
                    {item.label}
                  </Link>
                ))}
              </nav>
                    ...
                    <nav className="space-y-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                      {menuItems.map((item, index) => (
                        <Link
                          ...
                          className={`${menuItemClass(item.path)} animate-menu-item menu-item-hover`}
