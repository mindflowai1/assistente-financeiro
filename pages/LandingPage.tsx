...
const chatMessages = [
    { type: 'user', content: 'Oi! Gastei R$ 54,90 no supermercado.' },
    { type: 'bot', content: "Registrado! R$54,90 na categoria 'Alimentação'. 👍" },
    { type: 'user-audio', duration: '0:05' },
    { type: 'bot', content: "Entendido. Gasto de R$35,00 com 'Transporte' adicionado. Precisa de mais alguma coisa?" },
    { type: 'user', content: 'Me mostra um resumo dos meus gastos?' },
    {
        type: 'bot-list', content: 'Claro! Aqui está o resumo:', details: [
            { label: 'Supermercado', value: 'R$ 54,90' },
            { label: 'Transporte', value: 'R$ 35,00' },
            { divider: true },
            { label: 'TOTAL', value: 'R$ 89,90', isTotal: true }
        ]
    }
];
...
export const LandingPage: React.FC = () => {
    ...
    const submitPreCheckout = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Salvar dados no localStorage para pré-preenchimento na página de compra da Green
        if (preName || preEmail || prePhone) {
            const checkoutData = {
                name: preName || '',
                email: preEmail || '',
                phone: normalizePhone(prePhone || ''),
                timestamp: Date.now()
            };
            localStorage.setItem('greennCheckoutData', JSON.stringify(checkoutData));
        }

        // Usar os parâmetros padrão que a Green aceita (fn, em, ph)
        const params = new URLSearchParams();
        if (preName) params.append('fn', preName);
        if (preEmail) params.append('em', preEmail);
        if (prePhone) params.append('ph', normalizePhone(prePhone));

        const dest = params.toString() ? `${checkoutUrl}?${params.toString()}` : checkoutUrl;
                    ...
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
                        <div className="grid md:grid-cols-2 gap-12 items-center">
                            <div className="text-center md:text-left">
                                ...
                                <div className="block md:hidden mt-12 relative z-0">
                                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -z-10">
                                        <div className="w-[260px] h-[490px] bg-green-500/80 rounded-[3rem] blur-2xl animate-glow-pulse"></div>
                                    </div>
                                    <div className="relative mx-auto border-gray-700 bg-gray-800 border-[8px] rounded-[2.5rem] h-[450px] w-[225px] shadow-xl">
                                        ...
                                        <div className="w-full h-full overflow-hidden rounded-[2rem] bg-gray-900 flex flex-col pt-6">
                                            ...
                                            <div className="p-2 flex-grow overflow-y-auto flex flex-col justify-end">
                                                <div className="w-full space-y-2">
                                                    {chatMessages.map((msg, index) => (
                                                        <div
                                                            key={index}
                                                            className={`flex ${msg.type.startsWith('user') ? 'justify-end' : 'justify-start'} animate-chat-in`}
                                                            ...
                                                            {msg.type === 'user-audio' && (
                                                                <div className="bg-green-600 rounded-l-lg rounded-br-lg p-1.5 max-w-[80%] w-36">
                                                                    <div className="flex items-center gap-1.5">
                                                                        ...
                                                                        <span className="text-xs font-mono text-white">{msg.duration}</span>
                                                                    </div>
                                                                </div>
                                                            )}
                                                            {msg.type === 'bot-list' && (
                                                                <div className="bg-gray-700 rounded-r-lg rounded-bl-lg p-1.5 max-w-[80%] space-y-1">
                                                                    ...
                                                                    <div className="text-[10px] space-y-1 font-mono bg-gray-800/50 p-1.5 rounded-md">
                                                                        {msg.details?.map((detail, idx) => (
                                                                            detail.divider ? (
                                                                                <div key={idx} className="border-t border-gray-600 my-1"></div>
                                                                            ) : (
                                                                                <div key={idx} className={`flex justify-between ${detail.isTotal ? 'font-bold text-gray-100' : 'text-gray-300'}`}>
                                            ...
                                            <div className="bg-gray-800/80 p-1.5 flex items-center gap-1.5 flex-shrink-0">
                                                ...
                                                <div className="flex-grow h-8 bg-gray-700 rounded-full text-xs text-gray-500 px-3 flex items-center">Digite...</div>
                                                <svg className="w-5 h-5 text-gray-400" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                            ...
                            <div className="hidden md:block relative z-0">
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -z-10">
                                    <div className="w-[340px] h-[640px] bg-green-500/80 rounded-[3rem] blur-2xl animate-glow-pulse"></div>
                                </div>
                                {/* iPhone Mockup Frame */}
                        ...
                        <div className="grid md:grid-cols-2 gap-16 items-center">
                            ...
                            <div className="text-center md:text-left">
                                ...
                                <ul className="space-y-5 text-left">
                                    ...
                                    <li className="flex items-center gap-3">
                                        <svg className="w-6 h-6 text-green-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                                        <span className="text-gray-300"><span className="font-semibold text-white">Relatórios Completos:</span> Filtre suas transações por data ou categoria, <span className="text-green-400 font-semibold">exporte em PDF</span> para uma análise detalhada e tome o controle total.</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
